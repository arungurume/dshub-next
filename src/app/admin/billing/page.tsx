...
export default function BillingPage() {
  ...
  async function handlePlanUpgrade(planKey: string) {
    try {
      const { data } = await cmsApi.post(`/sac/subscriptions/${planKey}/checkout-session`, {});
            ...
            <div className="plans-grid">
              {plans.map(({ key: planKey, ...rest }) => (
                <PlanCard
                  ...
                  onSelect={() => handlePlanUpgrade(planKey)}
                />
              ))}
            </div>
          </section>
