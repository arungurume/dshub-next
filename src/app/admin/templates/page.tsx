...
function TemplatesPageInner() {
  ...
  // Tab state: 'canva' = My Canva Designs, 'saved' = My saved templates, 'dshub' = DSHub Canvas
  const [mainTab, setMainTab] = useState<'canva' | 'saved' | 'dshub'>('canva');
  ...
  // ── Saved/Draft templates state ────────────────────────────────────────────
  const [myTab, setMyTab] = useState<'saved' | 'draft'>('saved');
  const [myTemplates, setMyTemplates] = useState<Template[]>([]);
  const [myDrafts, setMyDrafts] = useState<Template[]>([]);
  ...
  // ── Handle OAuth callback (status=success in query params) ─────────────────
  useEffect(() => {
  ...
  // ── Canva: load profile + designs ─────────────────────────────────────────
  const loadCanvaProfile = useCallback(async () => {
    setCanvaLoading(true);
    try {
      const { data } = await cmsApi.get('/canva/profile');
      if (data?.profile) {
        setCanvaProfile(data.profile);
  ...
  const loadCanvaDesigns = useCallback(async () => {
    try {
      const { data } = await cmsApi.get('/canva/designs', { params: { size: 100 } });
  ...
  const filteredSaved = (myTab === 'saved' ? myTemplates : myDrafts).filter(
    (tpl) => !search || templateName(tpl).toLowerCase().includes(search.toLowerCase())
  );
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 0, paddingTop: 16 }}>
          {[
            ...
            { id: 'saved', icon: <BookmarkCheck size={15} />, label: t('TEMPLATES.tab_saved_templates') },
        ...
        {/* Sub-tabs for Saved */}
        {mainTab === 'saved' && (
          <div style={{ display: 'flex', gap: 0, paddingBottom: 12, paddingTop: 10 }}>
            {[
              { id: 'saved', label: t('TEMPLATES.tab_saved'), icon: <BookmarkCheck size={13} /> },
              { id: 'draft', label: t('TEMPLATES.tab_drafts'), icon: <FileText size={13} /> },
            ].map((st) => (
              <button
                key={st.id}
                onClick={() => setMyTab(st.id as any)}
                style={{
                  display: 'flex', alignItems: 'center', gap: 5, padding: '5px 14px',
                  borderRadius: 20, border: 'none',
                  background: myTab === st.id ? 'rgba(99,102,241,0.12)' : 'transparent',
                  color: myTab === st.id ? 'var(--accent)' : 'var(--text-muted)',
                  fontWeight: myTab === st.id ? 700 : 500, fontSize: '0.78rem',
              ...
              >
                ...
                <span style={{
                  ...
                  background: myTab === st.id ? 'var(--accent)' : 'var(--border)',
                  color: myTab === st.id ? '#fff' : 'var(--text-muted)',
                  fontSize: '0.65rem', fontWeight: 700,
                }}>
                  {st.id === 'saved' ? myTemplates.length : myDrafts.length}
                </span>
              </button>
            ))}
          </div>
              ...
              <div style={{ columns: 'auto 220px', columnGap: 14 }}>
                {filteredCanva.map((design) => (
                  <CanvaDesignCard
                    key={design.id}
                    design={design}
                    onClick={() => router.push(`/admin/templates/canva/${design.id}`)}
                  />
                ))}
              </div>
            )}
          ...
          myLoading ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 24px', color: 'var(--text-muted)' }}>
              ...
              <p style={{ fontSize: '0.9rem', margin: 0 }}>
                {myTab === 'saved' ? t('TEMPLATES.no_saved_templates') : t('TEMPLATES.no_drafts_found')}
              </p>
            </div>
          ) : (
            <div style={{ columns: 'auto 220px', columnGap: 14 }}>
